...
REQUIRED_SPACE_GB=2
SKIP_VENV=false
PERFORMANCE_TIER="minimal" # minimal, standard (cpu), high (cuda)
...
echo "✅ Selected Tier: $PERFORMANCE_TIER"
...
else echo "❌ docker-compose or podman-compose is required."; exit 1; fi

# Verify engine
if ! timeout 15s $COMPOSE_CMD ps >/dev/null 2>&1; then
    echo "⚠️ Warning: Container engine is not responding. Attempting restart..."
...
if [ "$SKIP_VENV" = false ]; then
    ...
    case "$PERFORMANCE_TIER" in
        ...
        "high")
            # To sync CUDA locally, user needs to remove the CPU index from pyproject.toml
            # We'll just sync with semantic extra for now.
